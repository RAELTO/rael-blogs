...
export function useMyReaction(boxId: string, userId?: string) {
  return useQuery({
...
export function useReactionCounts(boxId: string) {
  return useQuery({
...
export function useToggleReaction(boxId: string) {
  ...
  return useMutation({
    mutationFn: async ({ userId, type, current }: { userId: string; type: ReactionType; current: ReactionType | null }) => {
      if (current === type) {
        await supabase.from('box_reactions').delete().eq('box_id', boxId).eq('user_id', userId)
      } else if (current) {
        await supabase.from('box_reactions').update({ reaction_type: type }).eq('box_id', boxId).eq('user_id', userId)
      } else {
        await supabase.from('box_reactions').insert({ box_id: boxId, user_id: userId, reaction_type: type })
      }
    },
